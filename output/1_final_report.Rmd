...
Next, the restaurants from Lucerne are scraped using the Google Places API.

The issue here is that the API only lets us scrape 20 restaurants at a time. 
The other issue is that scraping places tagged as restaurants will also scrape other establishments, such as hotels, since usually there are many tags in the 'types' section. 
